  ...
  const selectLadWithCamera = (ladCode: string, event: any) => {
    ...
    if (map && bounds?.isValid?.()) {
      try {
        ...
        // Fall through to immediate selection if Leaflet is between layer states.
      }
    }
    onSelectLad(ladCode)
  }
  ...
  const wardOnEachFeature = (feature: GeoFeature, layer: Layer) => {
    ...
    const popupLines = sorted
      .map(entry => {
        ...
        return `<div>${entry.party}: ${Math.round(entry.value)}% expected vote share${suffix}</div>`
      })
      .join('')
    const prev = projection.prevWinner ? `${previousWinnerLabel}: ${projection.prevWinner}` : null
    layer.bindPopup(
      `<div class="poll-ward-popup">
        <div class="poll-ward-popup__title">${wardName}</div>
        <div class="poll-ward-popup__section">${popupLines}</div>
        <div class="poll-ward-popup__section">Seats up: ${vacancies}</div>
        ${prev ? `<div class="poll-ward-popup__section">${prev}</div>` : ''}
      </div>`,
      { autoPan: false }
    )
  }
