...
export default function TopNav({ title, items, subtitle, subtitleStyle }: TopNavProps) {
  ...
  const resolvedSubtitle = subtitle ?? ROUTE_SUBTITLES[router.pathname] ?? (title !== 'Poll of Polls' ? title : null)
  ...
  return (
    <header className="poll-topnav">
      ...
      {resolvedSubtitle ? (
        <div className="poll-topnav__subtitle" style={subtitleStyle ?? DEFAULT_SUBTITLE_STYLE}>
          {resolvedSubtitle}
        </div>
      ) : null}
    </header>
