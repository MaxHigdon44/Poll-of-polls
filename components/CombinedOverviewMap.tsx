...
function FitFeature({ feature }: { feature: any }) {
  ...
  useEffect(() => {
    try {
      if (!feature || !(map as any)._loaded) return
              ...
              onEachFeature={(feature, layer) => {
                ...
                const popupLines = sorted
                  .map(entry => {
                    ...
                    return `${entry.party}: ${Math.round(entry.value)}% expected vote share${suffix}`
                  })
                  .join('<br/>')
                layer.bindPopup(`<div class="poll-ward-popup"><strong>${wardName}</strong><br/>${popupLines}<br/>Seats up: ${vacancies}${projection.prevWinner ? `<br/>Incumbent: ${projection.prevWinner}` : ''}</div>`)
              }}
            />
          )}
        </>
