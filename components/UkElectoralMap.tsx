...
function FitToView({
  ...
  useEffect(() => {
    try {
      ...
      // Leaflet can briefly reference removed DOM nodes during route/layer swaps.
    }
      ...
      {view === 'overview' && countriesGeo && (
        <GeoJSON
          ...
          style={feature => {
            ...
            const fillColor = COUNTRY_WINNER_COLORS[name] || '#c4c4c4'
            return {
              color: isNi ? 'transparent' : '#1f2a44',
              weight: isNi ? 0 : 2.2,
              fillColor: isNi ? 'transparent' : fillColor,
