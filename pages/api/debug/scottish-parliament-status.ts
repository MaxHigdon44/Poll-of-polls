...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      constituencyCount: row.payload.constituencyRows?.length || 0,
      regionCount: Object.keys(row.payload.regionalSeatsByRegion || {}).length,
      seatPartyCount: Object.keys(row.payload.combinedSeatCounts || {}).length,
    })
  } catch (err) {
    console.error(err)
