...
const COUNTRY_KEY_BY_LABEL: Record<string, 'england' | 'scotland' | 'wales'> = {
  England: 'england',
  Scotland: 'scotland',
  Wales: 'wales',
}
...
export default function ElectoralMapsPage() {
  ...
  const countryWinnerColors = useMemo(() => {
    const colors: Partial<Record<'england' | 'scotland' | 'wales', string>> = {}
    countrySummaries.forEach(summary => {
      const countryKey = COUNTRY_KEY_BY_LABEL[summary.country]
      const leadingParty = summary.rows.find(row => row.party !== 'No overall control')?.party
      if (countryKey && leadingParty && SUMMARY_COLORS[leadingParty]) {
        colors[countryKey] = SUMMARY_COLORS[leadingParty]
      }
    })
    return colors
  }, [countrySummaries])
